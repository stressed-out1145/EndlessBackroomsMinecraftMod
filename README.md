Modrinth: https://modrinth.com/mod/endless_backrooms

Bilibili video: https://www.bilibili.com/video/BV1BPcizJEjp?spm_id_from=333.788.videopod.sections&vd_source=9c170d24e08bc904c5505ea646542aab


**⚠️ 1. Disclaimer & Safety Warning**

This mod contains **mild horror elements** (sound effects, visuals, etc.). If you feel uncomfortable while playing, please exit the game immediately. The author is not responsible for any psychological or physical discomfort caused by playing this mod. Please ensure you are in a suitable mental and physical condition to play such content.

**⚠️ 2. Copyright & License Notice**

All rights to this mod (Endless Backrooms) belong to stressed_out.
Without explicit permission, modifying the mod's code/assets, redistributing it, or using it for any commercial purposes (including but not limited to operating commercial servers or paid modpacks) is strictly prohibited.

**⚠️ 3. Intellectual Property Notice**

The "The Backrooms" IP does not belong to this project. This mod is an unofficial fan-made creation. All related IP rights belong to their respective original creators. The author claims no ownership of said IP.

**Downloading, installing, or playing this mod constitutes your acceptance of all the terms stated above.**

**⚠️ 1. 免责声明与安全警告**

本模组含有**微量恐怖元素**（音效、画面等）。若您在游玩过程中感到不适，请立即退出游戏。作者不对因游玩本模组导致的任何心理或生理不适承担任何责任。请确保您的身心状况适合游玩此类内容。

**⚠️ 2. 版权与许可声明**

本模组（Endless Backrooms）的所有权归 stressed_out 所有。
未经许可，严禁任何人修改模组代码/资源、进行二次分发，或使用本模组进行任何形式的商业用途（包括但不限于开设商业服务器、付费整合包等）。

**⚠️ 3. 知识产权说明**

“后室（The Backrooms）”IP 不属于本项目。本模组仅为粉丝创作的非官方作品，相关 IP 版权归其原始创作者所有。作者不声称拥有该 IP 的任何所有权。

**下载、安装或游玩本模组，即视为您已阅读并同意以上所有条款。**

**Introduction**

Welcome to Endless Backrooms, a survival horror mod that brings the Backrooms into Minecraft. Start your journey in the iconic Level 0, gather resources, and try to survive the entities lurking in the shadows. Can you find the way out, or will you be lost forever?

**How to start**

Create a new world in Minecraft. Then, find a way to get stuck in blocks. Upon dying from suffocate, you will noclip directly into Level 0. Good luck, Wanderer.

Levels:

level 0

level 1 (0.2.0):
- Aquila Sector (0.2.0)
- Gild Sector (0.2.3)

**Community & Supports**

The backrooms website referenced by this mod(en): [The Backrooms Wiki](https://backrooms-wiki.wikidot.com/)

A simple mod guide(en) : [Endless Backrooms -- Mod Guide](https://docs.qq.com/doc/DSVRSQkpad0FOWUVE)

Issues report: [Issues tracker (github)](https://github.com/stressed-out1145/EndlessBackroomsMinecraftMod/issues)

**引言**

欢迎来到 Endless Backrooms (无尽后室)，这是一款将后室带入 Minecraft 的生存恐怖模组。从标志性的 Level 0 开始你的旅程，收集资源，并试图在潜伏于阴影中的实体手下存活。你能找到出路，还是将永远迷失？

**如何开始**

在 Minecraft 中创建一个新世界。设法让自己窒息（被方块卡住）。当你因窒息而死亡时，你将直接卡入 Level 0。祝你好运，流浪者。

层级:

level 0

level 1 (0.2.0):
- Aquila Sector (0.2.0)
- Gild Sector (0.2.3)

**社区 & 支持**

本模组所参考的后室资料站(zh)：[The Backrooms中文维基](https://backrooms-wiki-cn.wikidot.com/)

简易的模组游玩教程(zh)：[《无尽的后室》模组游玩教程](https://docs.qq.com/doc/DSWVPWHNQcG1iVVNZ)

B站模组介绍视频(zh)：[《无尽的后室》模组介绍视频](https://www.bilibili.com/video/BV1BPcizJEjp/?spm_id_from=333.1387.homepage.video_card.click&vd_source=9c170d24e08bc904c5505ea646542aab)

问题/bug反馈: [反馈渠道 (github)](https://github.com/stressed-out1145/EndlessBackroomsMinecraftMod/issues)

MCMOD百科站(zh) [《无尽的后室》模组教程(mcmod上)](https://www.mcmod.cn/class/25989.html)

QQ群：666715026
